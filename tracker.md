...
- **v1.4.0b3** (2026-09-22): b1/b2 published. b3 fixes the one blocker found live — `dlux update` restarted the migration applier instead of recreating it, so Compose skipped its `pre_start` and any release with a migration rolled back on every pre-1.8.14-scaffold project. Also in 1.4.0: channel-aware resident checks, `check-policy.json` interval (default 900 s), check-now requests, `check --fix` exit 0 after repair, `resident-commands` check. No retirements (§9 inventory postponed to 1.5.0).
