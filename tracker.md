...
- **v1.5.3b1 (beta, published 2026-09-25)**: start/update `--skip-config` injects `DLUX_SKIP_CONFIG_IMPORT=True` into runtime service environments (DjangoLux 1.9.4b1+, whose manifest requires `>=1.5.3b1`); migrations and manual imports continue. Repeat on redeploy until setup completes. Docker Hub `:v1.5.3b1` + `:beta`, GitHub prerelease; `:latest` stays 1.5.2. Accepted on `testbed-dlux` as a local build and again as the published image.
...
- `composer dlux update` without `--version` installs the newest **stable** release even on a beta-channel deployment: `apply_package_update()` calls `source.obtain(target_version, workdir=...)` with no channel, while `--dry-run` and `dlux check` pass `_resolved_channel(runtime)`. The Options card is unaffected (it sends an explicit version). Found on `testbed-dlux` 2026-09-25; present since channels (1.3.14b1).
...
- 2026-09-25: `testbed-dlux` — candidate 1.5.3b1 (local build as `:beta`) with DjangoLux 1.9.4b1: `--skip-config` reached web/celery/agent/executor, a plain deploy unset it, `check` 15/15 with deployer == resident; published 1.5.3b1 then installed 1.9.4b1 inline with `dlux update --version`.
