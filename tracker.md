...
- **v1.5.0b1 published** (2026-09-23): `composer/ops.py` answers DjangoLux's Operations card — one named operation per request, token-matched result, phase 1 is read-only `check` via the extracted `collect_checkup()`. Live-accepted with DjangoLux 1.9.1b1 on the decrees stack; `:beta` is 1.5.0b1, `:latest` stays 1.4.1.
