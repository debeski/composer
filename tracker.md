...
- Verified on 2026-06-26 (v1.1.3): `python3 -m compileall composer` OK; argparse unit test confirms `-uo` resolves distinctly from `-u` (no prefix collision) and all three flags accept optional `[SERVICE]`; `--help`/`--version` OK; render smoke-test exercised restart-mode panel ("Restart Services (svc)", no Pull/Post-Start rows) and scoped-update panel ("Start Compose (svc)"). Docker runtime path (actual pull/recreate/restart against a live project) still pending.

### One-line info about last time edited Docs:
- Edited `README.md` (flag table: `-u`/`-uo`/`-r`), `CHANGELOG.md` (new `## v1.1.3`), `tracker.md` on 2026-06-26 for the update/restart flags; bumped `VERSION`→`1.1.3`.
