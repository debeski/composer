...
- `composer/version.py` reads the repo-level `VERSION` file; the current repo version is `1.1.6` (`v1.0.0`→`v1.1.5` all tagged+published; `v1.1.6` in-progress, NOT yet tagged). ALWAYS re-run `git tag` before editing CHANGELOG — releases get tagged mid-session; never edit a tagged version's entry.
