...
- Live update-progress feature (proxy-served; all 3 phases DONE + tested):
  ...
  - [x] dlux v1.3.4: redesigned Updates card (options.html/updater.js/options.css) — app-version header + status badge (up-to-date/available/updating) + details grid (image ref, running digest, last updated, last registry check, framework version+latest). State endpoint adds `app_version` + `image` via `image_update.image_status_summary()` (reads composer `image-available.json` + `DluxImageUpdate` history); `app_version()` auto-reads `BASE_DIR/VERSION` (override `DLUX_APP_VERSION`). Ships INLINE (no switch_pos rebuild). 45 tests + summary + JS parse pass.
  - [ ] RELEASE/DEPLOY: composer `v1.1.6`/`v1.1.7` + dlux `v1.3.3`/`v1.3.4` are released per user (all tagged). Card redesign (1.3.4) delivers via inline dlux update — no switch_pos rebuild. Optional: set `DLUX_APP_VERSION` in a project to override the auto app-version.
