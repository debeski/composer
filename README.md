...
`composer agent check [--json] [--no-publish] [--availability-file PATH] [-f FILE] [-d] [IMAGE ...]` checks image availability without pulling, recreating, or enabling maintenance. With no explicit images or output path, a deployment check reads the resident agent/updater's watched images and availability path from resolved Compose configuration, then atomically publishes the result through that running service. This refreshes the shared document the DLUX UI reads immediately instead of waiting for the hourly registry check; the UI displays it on its next refresh. It works from the deployment wrapper even though the wrapper does not mount the runtime volume. `--no-publish` opts out. Explicit image arguments remain diagnostic; `--availability-file PATH` instead writes to that path in the CLI's filesystem. Outside a configured deployment, image discovery falls back to `COMPOSER_CHECK_IMAGE`, `WEB_IMAGE`, then the Compose agent block. `-f FILE` selects an alternate Compose file; `-d` includes the development override when using default Compose files. `--json` emits the availability document, including optional candidate `version` and release `manifest` metadata. Exit `0` means every registry lookup and any requested publication succeeded, even when an update exists; exit `1` means a lookup was unknown, configuration discovery failed, or publication failed (for example, the resident service is stopped); exit `2` means invalid images or no image was found. Digest-pinned references are rejected. `COMPOSER_REGISTRY_TOKEN`, `COMPOSER_VERSION_LABEL`, and `COMPOSER_RELEASE_MANIFEST_LABEL` retain their existing meanings.

`composer.ops` answers DjangoLux's **Operations card**: a request file naming one operation from its table (`check`, which also previews the repairs it would apply, `check-fix-apply`, and `agent-update`) is performed by the resident agent and answered with a token-matched result on the runtime volume. An apply carries the digest of the files its preview read and is refused if they have changed since, so the repair that lands is the one the operator confirmed. An operation outside that table is refused rather than attempted, and the request carries no command, path or flags. See [docs/agent-protocol-v1.md](docs/agent-protocol-v1.md).
...
`watch` can also **detect a newer image** and publish availability for another process to act on: `--check-image IMAGE` (repeatable) + `--availability-file PATH` (and `--check-interval SECONDS`, default 900; DjangoLux 1.9.0+ overrides it with the interval chosen in Options, published to `state/check-policy.json`) poll the registry's tag digest vs the locally-pulled one and write `{ "available": …, "images": [ … ] }`. It only reports *readable* differences (an unreachable registry is "unknown", never a false positive), needs no registry access from the consumer, and re-checks right after an applied update. `COMPOSER_REGISTRY_TOKEN` covers private repositories.
